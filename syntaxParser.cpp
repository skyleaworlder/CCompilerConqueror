    ...
    while(std::getline(in, line_read, '\n')) {
        ...
        for (auto right_symbols : right_symbols_multi_derives) {
            ...
            if (line_read[0] == '!') {
                ...
                // convert type std::vector to std::unordered_set
...
std::pair<bool, std::unordered_set<int>> BaseGrammar::mergeSet(const std::unordered_set<int> first, const std::unordered_set<int> second, bool with_epsilon)
{
	bool success = false;
	if (first == second)
	...
	std::unordered_set<int> temp_first, temp_second;
